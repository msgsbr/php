    ...
    try{
        ...
        foreach ($_FILES as $k=>$file){
            ...
            if (is_uploaded_file($tmp_filename)) {
                $target_file = $date.".".pathinfo($file['name'], PATHINFO_EXTENSION);
                $origem = $file['tmp_name'];

                if (move_uploaded_file($origem, $uploaddir['pathFull'] . "/".$target_file ) ) {
                    $result->success = true;
                    $result->message = "Arquivo recebido com sucesso!";
                    $result->content = $uploaddir['pathImg']."/".$target_file;
                    unlink($origem);
                } else
...
private function gerarDirName($idEA1){
    ...
    $return = array();
    $return['pathImg'] = "/imagens".$pathEmp.$pathAssinatura;
